...
function createLobbies({now=()=>Date.now(),limit=8,snapshot,maintenance=false}={}){
 const rooms=new Map(),sessions=new Map(),attempts=new Map();let maintenanceAt=maintenance?now():null;
 const clock=()=>maintenanceAt??now();
 function setMaintenance(enabled){if(typeof enabled!=='boolean')throw Error('Invalid maintenance state');if(enabled&&maintenanceAt===null){maintenanceAt=now();for(const r of rooms.values())r.room.setMaintenance(true);}else if(!enabled&&maintenanceAt!==null){const duration=Math.max(0,now()-maintenanceAt);for(const r of rooms.values()){r.seen+=duration;if(r.endedAt!==undefined)r.endedAt+=duration;r.room.setMaintenance(false);}for(const a of attempts.values())a.at+=duration;maintenanceAt=null;}}
 if(snapshot){
 ...
 if(maintenance)for(const r of rooms.values())r.room.setMaintenance(true);
 ...
 function cleanup(){const time=clock();for(const [code,r] of rooms)if((r.endedAt!==undefined&&time-r.endedAt>60000)||time-r.seen>(r.players===1?30*60000:2*60*60000)){rooms.delete(code);for(const token of r.tokens)sessions.delete(token);}for(const [id,a] of attempts)if(time-a.at>30*60000)attempts.delete(id);}
 ...
 function resolve(token){cleanup();const r=sessions.get(token);if(r)r.seen=clock();return r;}
 return {get maintenance(){return maintenanceAt!==null;},setMaintenance,checkpoint,claim(token,clientId,takeover){const r=resolve(token);return r?r.room.claim(token,clientId,takeover):{ok:false,reason:'unauthorized'};},leave(token){const r=resolve(token);if(!r)return {ok:false,reason:'unauthorized'};const result=r.room.leave(token);if(result.ok)r.endedAt??=clock();return result;},touch(token){const r=resolve(token);return r?r.room.touch(token):false;},create:body=>request('create',body),join:body=>request('join',body),view(token){const r=resolve(token);if(!r)return null;return {...r.room.view(token),lobby:{code:r.code,players:r.players}};},receive(token,packet){const r=resolve(token);if(!r)return {ok:false,reason:'unauthorized'};if(r.players<2)return {ok:false,reason:'waiting-for-partner'};return r.room.receive(token,packet);},tick(){cleanup();for(const r of rooms.values())if(r.players===2)r.room.tick();}};
}
