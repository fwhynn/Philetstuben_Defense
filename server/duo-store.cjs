...
function createStore(file,{now=()=>Date.now(),write=atomicWrite,maintenance=false}={}){
 ...
 return {receipts:token=>rooms.receipts(token),get maintenance(){return rooms.maintenance;},setMaintenance:enabled=>change('setMaintenance',enabled),get healthy(){return !failed;},flush,claim(token,clientId,takeover){if(failed)throw Error('Duo storage unavailable');const result=rooms.claim(token,clientId,takeover);if(result.changed)flush();return result;},leave:token=>change('leave',token),create:body=>change('create',body),join:body=>change('join',body),receive:(token,packet)=>change('receive',token,packet),view:token=>rooms.view(token),touch:token=>rooms.touch(token),tick(){if(failed)return;const settled=rooms.tick();if(settled||now()-lastSaved>=1000)try{flush();}catch{console.error('Duo storage failed; simulation stopped.');}}};
}
