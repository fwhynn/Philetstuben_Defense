...
function createStore(file,{now=()=>Date.now(),write=atomicWrite}={}){
 const snapshot=fs.existsSync(file)?JSON.parse(fs.readFileSync(file,'utf8')):undefined;
 const rooms=createLobbies({now,snapshot});let failed=false,lastSaved=now();
 ...
 return {get healthy(){return !failed;},flush,create:body=>change('create',body),join:body=>change('join',body),receive:(token,packet)=>change('receive',token,packet),view:token=>rooms.view(token),touch:token=>rooms.touch(token),tick(){if(failed)return;rooms.tick();if(now()-lastSaved>=1000)try{flush();}catch{console.error('Duo storage failed; simulation stopped.');}}};
}
