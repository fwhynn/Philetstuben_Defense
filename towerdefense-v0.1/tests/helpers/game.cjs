...
function load({headless=false,initialProfile=null,initialStorage={},rotateView=null}={}){
  ...
  const storage=new Map(Object.entries(initialStorage));if(initialProfile)storage.set('hex-bastion-profile-v1',JSON.stringify(initialProfile));const documentListeners={};
