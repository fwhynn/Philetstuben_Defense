...
function rules(){const c={};for(const name of ['random','map','run-runtime'])vm.runInNewContext(fs.readFileSync(path.join(__dirname,'../'+name+'.js'),'utf8'),c);return vm.runInNewContext('({rng:HexRandom,runtime:HexRunRuntime})',c);}
