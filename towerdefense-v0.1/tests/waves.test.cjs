...
test('regular waves keep growing past the old cap and boss gold is forecast exactly once',()=>{
  const {waves}=load();
  for(const wave of [1,3,9,11,19,21])assert.equal(waves.plan(wave).boss,null);
...
test('each tenth wave spawns one boss at a valid entrance with a route ending at base',()=>{
  for(const wave of [9,10,11,20,30]){
    const {a,waves}=prepare(wave),sources=a.spawnSources();a.startWave();a.startWave();
    const bosses=a.state.enemies.filter(e=>e.type==='boss');assert.equal(bosses.length,wave%10===0?1:0);
...
test('boss entrance repeats for a seed and varies across seeds',()=>{
  ...
  for(let i=0;i<20;i++){
    const pos=()=>{const {a}=prepare(10,'entrance-'+i);a.startWave();const e=a.state.enemies[0];return e.x+','+e.y;};
...
test('periodic and landmark bosses coexist; forecast accounts for both',()=>{
  ...
  assert.match(elements.get('goldForecast').textContent,/Maximal \+185 Gold/);
...
test('a defeated periodic boss grants gold and a working card reward only once',()=>{
  const {a,selectSlot,buyTower,elements}=prepare(10);
  ...
  assert.equal(a.state.deck.length,deck+1);assert.equal(a.state.phase,'reward');
