const HexBuildings=(()=>{
  ...
  function buy(state,slot,type){
    ...
    if(!def||!tile||!allowedTypes(tile).includes(type)||(type==='portal'&&state.phase!=='build')||!['place','build','wave'].includes(state.phase)||state.hp<=0||state.gold<def.cost||slot.index<0||slot.index>=(tile.buildingSlots||0)) return false;
