const HexBuildings=(()=>{
  ...
  function highlight(state){
    ...
    const covered=new Map();if(building.type!=='house')for(const t of state.map.values())for(const other of t.buildings||[])if(other&&other!==building&&other.type===building.type)for(const target of state.map.values())if(applies(t,target,other))covered.set(target.q+','+target.r,target);
  ...
  return {allowedTypes,upgrades,specials,definition,nextUpgrade,upgrade,canTarget,setTarget,definitions,highlight,buy,effects,cost,refresh,refund,sell};
})();
