  ...
  function trees(profile){
    ...
    for(const [type,tower] of Object.entries(HexData.TOWERS)){
      ...
      const paths=HexData.availableUpgrades({type}).map(([id,branch])=>{
        const final=HexData.availableUpgrades({type,branch:id})[0]?.[1],ultimate=HexData.ultimateDefinition({type,branch:id}),unlocked=HexProfile.ownsUltimate(profile,type,id),active=activeId===id;
        return {steps:[{tier:'Stufe 2',name:branch.name,desc:branch.desc,cost:branch.cost},final&&{tier:'Stufe 3',name:final.name,desc:final.desc,cost:final.cost}].filter(Boolean),
          meta:{tier:'Ultimate',name:ultimate.name,desc:ultimate.desc,cost:ultimate.cost,state:unlocked?(active?'active':'owned'):'locked',status:unlocked?(active?'✓ Aktiv für den nächsten Run':'✓ Freigeschaltet · inaktiv'):(owned?'Meta-Freischaltung':'Benötigt Turmfreischaltung'),
      ...
      list.push({type,kind:'tower',name:tower.name,desc:tower.desc,color:tower.color,cost:tower.cost,owned,paths,
        ...
        hint:'Pro Turm ist höchstens ein Ultimate aktiv. Wechseln kostet keine Diamanten.'});
    }
    for(const [type,def] of Object.entries(HexBuildings.definitions)){
      ...
      const steps=[0,1].map(i=>({tier:'Stufe '+(i+2),name:def.name+' · Stufe '+(i+2),desc:HexBuildings.definition({type,level:i+2}).desc,cost:HexBuildings.upgrades[type][i].cost}));
      list.push({type,kind:'building',name:def.name,desc:def.desc,color:BUILDING_COLOR,cost:def.cost,owned:true,
        paths:[{steps,meta:{tier:'Spezial',name:special.name,desc:special.desc,cost:special.cost,state:owned?'active':'locked',status:owned?'✓ Dauerhaft freigeschaltet':'Meta-Freischaltung',action:owned?null:{kind:'building',cost:HexProfile.BUILDING_UNLOCKS[type].cost}}}],
  ...
  function render(host,profile,onUnlock,rail=document.getElementById('arsenalTabs')){
    ...
    for(const tree of list){
      ...
      tree.paths.forEach((path,index)=>{
        ...
        for(const step of path.steps)column.appendChild(el('article','arsenalStep',`<span class="arsenalTier">${step.tier}</span><strong>${step.name}</strong><p>${step.desc}</p><span class="arsenalCost">Im Run: ${step.cost} 🪙</span>`));
        const m=path.meta,card=el('article','arsenalStep arsenalMeta '+m.state,`<span class="arsenalTier">${m.tier}</span><strong>${m.name}</strong><p>${m.desc}</p><span class="arsenalCost">Im Run: ${m.cost} 🪙 nach Stufe 3</span><small class="arsenalStatus">${m.status}</small>`);
