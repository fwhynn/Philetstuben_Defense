(() => {
  ...
  function render() {
    ...
    for (let player = 0; player < 2; player++) {
      ...
      if (s.rewardOffer) {
        const offer = s.rewardOffer; offer.choices.forEach((c, index) => {
  ...
  if (token) {
    ...
    let first = true, controlsKey = ''; remote = HexDuoClient.create({ token, onStatus: (text,session) => { document.getElementById('networkStatus').textContent = text;document.getElementById('takeoverSession').hidden=!session?.conflict;document.getElementById('leaveSession').disabled=!!session?.conflict||!!session?.ended; }, onView: view => { match = HexDuoClient.presentation(view, match); if (view.lobby) { document.getElementById('invitePanel').hidden = false; document.getElementById('inviteCode').textContent = 'Lobby ' + view.lobby.code + ' · ' + view.lobby.players + '/2 Spieler'; document.getElementById('inviteLink').value = location.origin + '/duo-lobby.html#join=' + view.lobby.code; } if (first) { first = false; activeBoard = view.player; switchBoard(activeBoard); } const own = view.boards[view.player], key = JSON.stringify([view.wave, view.hp, view.phase, view.lobby, view.connection?.maintenance,view.connection?.paused,view.connection?.expired,view.connection?.ended,view.ready, view.boards.map(b => b.phase), view.pendingHelp, view.portals, view.reinforcements, own.phase, own.gold, own.hand, own.rewardOffer, own.celebration, own.map]); if (key !== controlsKey) { controlsKey = key; render(); } else renderers[activeBoard].render(match.boards[activeBoard].state, targets(match.boards[activeBoard].state)); } });
