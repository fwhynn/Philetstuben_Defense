const HexTutorial=(()=>{
  const steps=[
    ...
    'Klicke deinen gebauten Turm an und verbessere ihn. Beim Bogenschützen kostet die erste Verbesserung 35 Gold und hilft dir in der ersten Welle.',
  ...
  function begin(state){const towers=[...state.map.values()].flatMap(t=>t.towers||[]).filter(Boolean);return {active:true,step:towers.some(t=>t.level>1||t.branch)?5:towers.length?4:state.phase==='place'?0:2};}
  function advance(tutorial,event){
    if(!tutorial?.active)return false;
    const target={rotate:1,place:2,slot:3,buy:4,upgrade:5,health:6,wave:7}[event];
