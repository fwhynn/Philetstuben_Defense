...
const HexModelMap=(()=>{
  // Straßenkanten der Grundformen bei Rotation 0 (wie in docs/ASSET_SPEC.md / data.js).
  ...
  // Belohnungskarten mit eigener Optik statt Alias auf eine Grundform (siehe docs/ASSET_SPEC.md, Abschnitt 3.5).
  ...
  function modelFor(tile){
    ...
    if(tile.type==='base') return {name:'base',rotation:0,   // der Renderer wählt die Festung (tile_base_<heldId>), 'base' ist der Ersatz
      ...(tile.roads?.length===2?{proceduralRoads:true}:{})};
  ...
  return {SHAPES,TILE_MODELS,NAMED_TILES,PROCEDURAL_ROAD_TILES,FORTRESSES,ALL_MODELS,matchShape,modelFor,propAngle};
})();
