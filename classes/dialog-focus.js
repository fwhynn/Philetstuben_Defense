...
(()=>{
 ...
 function sync(){
   for(const dialog of dialogs){if(visible(dialog)&&!open.has(dialog)){previous.set(dialog,document.activeElement);open.add(dialog);const target=dialog.querySelector('[data-dialog-initial-focus]')||dialog;target.setAttribute?.('tabindex','-1');target.focus?.();}else if(!visible(dialog)&&open.delete(dialog)){const opener=previous.get(dialog);if(opener?.isConnected&&opener.getClientRects().length)opener.focus?.();previous.delete(dialog);}}
 }
 new MutationObserver(sync).observe(document.body,{subtree:true,attributes:true,attributeFilter:['class','aria-modal']});
 document.addEventListener('keydown',event=>{if(event.key!=='Tab')return;const dialog=[...open].at(-1);if(!dialog)return;const items=controls(dialog);if(!items.length){event.preventDefault();return;}const first=items[0],last=items.at(-1);if(event.shiftKey&&(document.activeElement===first||!items.includes(document.activeElement))){event.preventDefault();last.focus();}else if(!event.shiftKey&&(document.activeElement===last||!items.includes(document.activeElement))){event.preventDefault();first.focus();}});
 sync();
})();
