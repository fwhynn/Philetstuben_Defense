...
function load({ headless = false, initialProfile = null, initialStorage = {}, rotateView = null } = {}) {
  ...
  for (const file of ['biomes.js', 'data.js', 'ui-layout.js', 'tutorial.js', 'heroes.js', 'profile.js', 'map.js', 'random.js', 'waves.js', 'combat.js', 'rewards.js', 'deck.js', 'buildings.js', 'exploration.js', 'camera.js', 'svg-renderer.js', 'arsenal.js']) vm.runInNewContext(fs.readFileSync(path.join(classesDir, file), 'utf8'), context);
