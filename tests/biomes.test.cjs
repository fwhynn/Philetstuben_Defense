...
function load() { const c = {}; for (const name of ['biomes', 'data', 'map', 'waves', 'combat']) vm.runInNewContext(fs.readFileSync(path.join(__dirname, '../classes', name + '.js'), 'utf8'), c); return vm.runInNewContext('({biomes:HexBiomes,data:HexData,map:HexMap,combat:HexCombat})', c); }
...
test('guardian resistance follows its origin and halves only the matching element including ultimate upgrades', () => {
  ...
  for (const [biome, type, branch, finalUpgrade] of [['ash', 'flame', null, null], ['ash', 'element', 'elementFire', 'elementVolcano'], ['storm', 'chain', null, null], ['storm', 'element', 'elementWind', 'elementTempest']]) {
    const damage = (resistant) => {
      const e = { type: 'boss', alive: true, hp: 1000, x: 20, y: 0, index: 0, t: 0, speed: 0, points: [{ x: 20, y: 0 }, { x: 500, y: 0 }], resistances: resistant ? biome === 'ash' ? { fire: .5 } : { lightning: .5, wind: .5 } : {} };
