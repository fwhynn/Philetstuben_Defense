...
test('daily uses fixed seeded conditions and lends towers without changing profile ownership',()=>{
  ...
  a.newRun(undefined,undefined,undefined,'2026-09-22');const seed=a.state.seed,hand=[...a.state.hand],exits=[...a.state.baseExits];
  ...
  a.state.wave=20;a.endWave();let profile=JSON.parse(storage.get('hex-bastion-profile-v1'));assert.equal(profile.diamonds,10);assert.ok(!profile.unlockedTowers.includes('ballista'));assert.equal(profile.records.highestWave,0);assert.equal(a.state.phase,'victory');
