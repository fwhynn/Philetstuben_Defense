...
test('research map pans without scrolling, zooms around the pointer and fits the viewport', () => {
  const fs = require('node:fs'), path = require('node:path'), vm = require('node:vm'), html = fs.readFileSync(path.join(__dirname, '../index.html'), 'utf8'); assert.ok(html.indexOf('src="classes/arsenal.js"') < html.indexOf("s.src = 'classes/game.js'") || html.indexOf('src="classes/arsenal.js"') < html.indexOf("s.src='classes/game.js'"));
