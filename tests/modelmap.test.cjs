...
test('tiles keep their stored rotation and base uses the base model', () => {
  ...
  assert.equal(models.modelFor({ type: 'base', roads: [0] }).name, 'base');
  assert.equal(models.modelFor({ type: 'base', roads: [0, 3] }).proceduralRoads, true);
});
...
test('roadless building cards have escalating rarity and independently positioned slots', () => {
  ...
  for (const [id, rarity, count] of [['buildingPlot', 'Uncommon', 1], ['buildingQuarter', 'Rare', 2], ['buildingDistrict', 'Epic', 3]]) {
    ...
    for (let rotation = 0; rotation < 6; rotation++) {
      const tile = { q: 0, r: 0, type: id, rotation, roads: [], slots: 0, buildingSlots: count }, positions = Array.from({ length: count }, (_, i) => map.buildingPosition(tile, i)); assert.equal(new Set(positions.map(p => p.x + ',' + p.y)).size, count); assert.equal(map.roadGeometry(tile).legs.size, 0); assert.equal(models.modelFor(tile).proceduralRoads, true);
